...
class TextEncoder(nn.Module):
    ...
    def forward(self, c, c_lengths, f0):
        c = c.transpose(1, 2)   # [b, in, t]
        c = self.hub(c)         # [b, h, t]
        
        x = self.lrelu(c * math.sqrt(self.hidden_channels))     # [b, h, t]
        x_mask = torch.unsqueeze(commons.sequence_mask(c_lengths, x.size(2)), 1).to(
            x.dtype
        )                                                       # [b, 1, t]
...
class SynthesizerTrn(nn.Module):
    ...
    def forward(self, ppg, vec, pit, spec, spk, vec_l, spec_l):
        ...
        # speaker
        # spk_preds = self.speaker_classifier(emb_c)
